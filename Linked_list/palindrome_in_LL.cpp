#include <bits/stdc++.h>
using namespace std;
class Node
{
public:
    int data;
    Node *next;

    Node(int data)
    {
        this->data = data;
        this->next = NULL;
    }
};
Node* reverse(Node *head)
{
    Node *prev = NULL;
    Node *curr = head;
    Node *next = curr;
    while (curr != NULL)
    {
        next = curr->next;
        curr->next = prev;
        prev = curr;
        curr = next;
    }
    return prev;
}
bool checkPalindrome(Node *head)
{
    if (head == NULL)
    {
        cout << "LL is empty :"<<endl;
        return true;
    }
    if (head->next = NULL)
    {
        // only  1 node;
        return true;
    }
    // > 1 node in LL

    // step 1: find middle node
    Node *slow = head;
    Node *fast = head->next;

    while (fast != NULL)
    {
        fast = fast->next;
        if (fast != NULL)
        {
            fast = fast->next;
            slow = slow->next;
        }
    }
    // slow pointer is palindrome to the middle node

    // step b  reverse LL is after middle/slow node;
    Node* reverseLLhead = reverse(slow -> next);
   // join the reverse LL into the left part
    slow->next = reverseLLhead;

    // step C: start comparison

    Node *temp1 = head;
    Node *temp2 = reverseLLhead;
    while (temp2 != NULL)
    {
        if (temp1->data != temp2->data)
        {
            // Not a palindome
            return false;
        }
        else
        {
            // if data is equal , then badh jao
            temp1 = temp1->next;
            temp2 = temp2->next;
        }
    }
    return true;
}
int main()
{
    Node *head = new Node(10);
    Node *second = new Node(20);
    Node *thied = new Node(30);
    Node *forth = new Node(20);
    Node *fiveth = new Node(10);

    head->next = second;
    second->next = thied;
    thied->next = forth;
    forth->next = fiveth;
    fiveth->next = NULL;

    bool isPalindrome = checkPalindrome(head);

    if (isPalindrome)
    {
        cout << "LL  is valid palindrome " << endl;
    }
    else
    {
        cout << "LL is not a palindrome" << endl;
    }

    return 0;
}